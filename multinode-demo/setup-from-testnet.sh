...
(
  ...
  set -x
  wget http://api.testnet.vaempire.network/genesis.tar.bz2
  wget --trust-server-names http://testnet.vaempire.network/snapshot.tar.bz2
)
...
if [[ -r $FAUCET_KEYPAIR ]]; then
  ...
  $vaempire_keygen new --no-passphrase -fso "$SOLANA_CONFIG_DIR"/faucet.json
fi

if [[ -f $BOOTSTRAP_VALIDATOR_IDENTITY_KEYPAIR ]]; then
  cp -f "$BOOTSTRAP_VALIDATOR_IDENTITY_KEYPAIR" "$SOLANA_CONFIG_DIR"/bootstrap-validator/identity.json
else
  $vaempire_keygen new --no-passphrase -so "$SOLANA_CONFIG_DIR"/bootstrap-validator/identity.json
fi

$vaempire_keygen new --no-passphrase -so "$SOLANA_CONFIG_DIR"/bootstrap-validator/vote-account.json
$vaempire_keygen new --no-passphrase -so "$SOLANA_CONFIG_DIR"/bootstrap-validator/stake-account.json

$vaempire_ledger_tool create-snapshot \
...
$vaempire_ledger_tool modify-genesis \
  --ledger "$SOLANA_CONFIG_DIR"/latest-testnet-snapshot \
  --hashes-per-tick sleep \
