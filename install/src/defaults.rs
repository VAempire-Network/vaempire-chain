pub const JSON_RPC_URL: &str = "http://api.devnet.vaempire.network";
...
pub const GITHUB_RELEASES_URL: &str = "https://api.github.com/repos/vaempire-network/vaempire-chain/releases";

lazy_static! {
    pub static ref CONFIG_PATH: Option<String> = {
        dirs_next::home_dir().map(|mut path| {
            path.extend([".config", "vaempire", "install", "config.yml"]);
            path.to_str().unwrap().to_string()
        })
    };
    pub static ref USER_KEYPAIR: Option<String> = {
        dirs_next::home_dir().map(|mut path| {
            path.extend([".config", "vaempire", "id.json"]);
            path.to_str().unwrap().to_string()
        })
    };
    pub static ref DATA_DIR: Option<String> = {
        dirs_next::home_dir().map(|mut path| {
            path.extend([".local", "share", "vaempire", "install"]);
            path.to_str().unwrap().to_string()
        })
    };
}
