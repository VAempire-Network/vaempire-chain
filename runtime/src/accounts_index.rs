...
use vaempire_account_program::{VAccountStorage, VaempireAccountType};
...
pub enum IndexKey {
    ...
    VaempireAccountStorage(Pubkey),
    VaempireAccountOwner(Pubkey),
    VaempireAccountOperational(Pubkey),
    VaempireRelyingOwner(Pubkey),
}
...
pub enum AccountIndex {
    ...
    SplTokenMint,
    SplTokenOwner,
    VaempireAccountStorage,
    VaempireAccountOwner,
    VaempireAccountOperational,
    VaempireRelyingOwner,
}
...
impl AccountIndex {
    pub fn name(&self) -> &str {
        match self {
            ...
            AccountIndex::VaempireAccountStorage => "Vaempire Account Storage",
            AccountIndex::VaempireAccountOwner => "Vaempire Account Owner",
            AccountIndex::VaempireAccountOperational => "Vaempire Account Operational",
            AccountIndex::VaempireRelyingOwner => "Vaempire Relying Owner",
        }
    }
}
...
pub struct AccountsIndex<T: IndexValue> {
    ...
    // Vaempire Indices
    vaempire_account_storage_index: SecondaryIndex<DashMapSecondaryIndexEntry>,
    vaempire_account_owner_index: SecondaryIndex<DashMapSecondaryIndexEntry>,
    vaempire_account_operational_index: SecondaryIndex<DashMapSecondaryIndexEntry>,
    vaempire_relying_party_owner_index: SecondaryIndex<DashMapSecondaryIndexEntry>,
...
impl<T: IndexValue> AccountsIndex<T> {
    ...
    pub fn new(config: Option<AccountsIndexConfig>) -> Self {
        ...
        Self {
            ...
            //Vaempire indexes
            vaempire_account_storage_index: SecondaryIndex::<DashMapSecondaryIndexEntry>::new("vaempire_account_storage_index"),
            vaempire_account_owner_index: SecondaryIndex::<DashMapSecondaryIndexEntry>::new("vaempire_account_owner_index"),
            vaempire_account_operational_index: SecondaryIndex::<DashMapSecondaryIndexEntry>::new("vaempire_account_operational_index"),
            vaempire_relying_party_owner_index: SecondaryIndex::<DashMapSecondaryIndexEntry>::new("vaempire_relying_party_owner_index"),
        }
    }
        ...
        R: RangeBounds<Pubkey> + std::fmt::Debug,
    {
        ...
        */
        match scan_type {
            ...
            ScanTypes::Indexed(IndexKey::VaempireAccountStorage(va_storage_key)) => self
                .do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.vaempire_account_storage_index,
                    &va_storage_key,
                    Some(max_root),
                    config,
                ),
            ScanTypes::Indexed(IndexKey::VaempireAccountOwner(va_owner_key)) => self
                .do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.vaempire_account_owner_index,
                    &va_owner_key,
                    Some(max_root),
                    config,
                ),
            ScanTypes::Indexed(IndexKey::VaempireAccountOperational(va_operational_key)) => self
                .do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.vaempire_account_operational_index,
                    &va_operational_key,
                    Some(max_root),
                    config,
                ),
            ScanTypes::Indexed(IndexKey::VaempireRelyingOwner(va_owner_key)) => self
                .do_scan_secondary_index(
                    ancestors,
                    func,
                    &self.vaempire_relying_party_owner_index,
                    &va_owner_key,
                    Some(max_root),
                    config,
                ),
    ...
    pub(crate) fn update_secondary_indexes(
        ...
        // Vaempire account
        if *account_owner == vaempire_account_program::id() {
            match VaempireAccountType::try_from(account_data) {
                Ok(VaempireAccountType::Account(account_info)) => {
                    if account_indexes.contains(&AccountIndex::VaempireAccountStorage) {
                        let storage = account_info.find_storage_key(pubkey);
                        self.vaempire_account_storage_index.insert(&storage, pubkey);
                    }

                    if account_indexes.contains(&AccountIndex::VaempireAccountOwner) {
                        for owner in account_info.owners {
                            if owner != Pubkey::default() {
                                self.vaempire_account_owner_index.insert(&owner, pubkey);
                            }
                        }
                    }
                }
                Ok(VaempireAccountType::Storage(VAccountStorage { operationals })) => {
                    if account_indexes.contains(&AccountIndex::VaempireAccountOperational) {
                ...
                Err(err) => log::warn!("Unable to parse Vaempire Account: {:?}", err),
            }
        }

        // Vaempire relying party data
        if *account_owner == vaempire_relying_party_program::id() {
            match RelyingPartyData::try_from(account_data) {
                Ok(acc) => {
                    if account_indexes.contains(&AccountIndex::VaempireRelyingOwner) {
                        self.vaempire_relying_party_owner_index
