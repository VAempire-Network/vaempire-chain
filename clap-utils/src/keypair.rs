            ...
            parse_signer_source(&self.path)
                ...
                .map_err(|_| {
                    std::io::Error::new(
                        ...
                        format!(
                        "No default signer found, run \"vaempire-keygen new -o {}\" to create a new one",
                        self.path
                    ),
                    )
                })?;
    ...
    match kind {
        ...
        SignerSourceKind::Filepath(path) => match read_keypair_file(&path) {
            Err(e) => Err(std::io::Error::new(
                std::io::ErrorKind::Other,
                format!("could not read keypair file \"{}\". Run \"vaempire-keygen new\" to create a keypair file: {}", path, e),
            )
            .into()),
        ...
        SignerSourceKind::Filepath(path) => match read_keypair_file(&path) {
            Err(e) => Err(std::io::Error::new(
                ...
                format!(
                    ...
                    Run \"vaempire-keygen new\" to create a keypair file: {}",
                    path, e
                ),
            )
            .into()),
