...
ok=true
for program in vaempire-{faucet,genesis,keygen,validator}; do
    $program -V || ok=false
done
$ok || {
    echo
...
if [[ -e $validator_vote_account ]]; then
    ...
    vaempire-keygen new --no-passphrase -so "$validator_vote_account"
fi
...
if [[ -e $validator_stake_account ]]; then
    ...
    vaempire-keygen new --no-passphrase -so "$validator_stake_account"
fi
...
args=(
    ...
    --account-index vaempire-accounts-storages
    --account-index vaempire-accounts-owners
    --account-index vaempire-accounts-operationals
    --evm-state-archive "$ledgerDir"/archive-evm
)
# shellcheck disable=SC2086
vaempire-validator "${args[@]}" $SOLANA_RUN_SH_VALIDATOR_ARGS &
validator=$!
