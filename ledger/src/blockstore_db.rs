...
pub enum BlockstoreError {
    ...
    Other(&'static str), // TODO(vaempire): remove, use specific error variant
}
...
impl Rocks {
    fn open(path: &Path, options: BlockstoreOptions) -> Result<Rocks> {
        ...
        // this is only needed for LedgerCleanupService. so guard with PrimaryOnly (i.e. running vaempire-validator)
