...
echo "Vaempire commit is $LOCAL"
echo "Solana merging commit is $REMOTE"


vaempire_changes="$current_folder/vaempire-changes-$BASE-$LOCAL.txt"

remote_changes="$current_folder/vaempire-changes-remote-$REMOTE-$LOCAL.txt"
# echo "Forming list of files that differ in current branch from base version $vaempire_changes"
...
git diff $LOCAL..$BASE --numstat | cut -f3- | sed -r 's/\{(.*) => (.*)\}/\2/' |  grep -Ev '^(docs|explorer|web3\.js)'  | sort  > $vaempire_changes
...
keep_ours_file="$current_folder/vaempire-rewrite"

# Vaempire use multiple repositories instead of big mono-repo:
# 1. vaempire-chain is repository only for blockchain related stuff.
# 2. web3-js mooved into https://github.com/vaempire/web3.js
# 3. Explorer mooved into https://github.com/vaempire/native-explorer
# 4. Docs are mooved into https://github.com/vaempire/vaempire-chain-docs
# 5. account-benches
...
# Filter out changes from file that wasn't touched by vaempire changes

for file in $(comm -23 $remote_changes $vaempire_changes)
do
...
# sed -e 's/^/\:\^/' $vaempire_changes > $pathspec_file
