# Vaempire fork maintenance guide and tools

Vaempire is a fork of Solana blockchain.
...
**In Vaempire Some of Solana changes (especially security fixes) are ported dirrectly to Vaempire repository.**

In Vaempire we use `develop` branch as a single source of code.
...
At some point of time we do a "full-update" which merge all solana changes to the vaempire develop.
...
In this document we describe some common parts that can be treat differently, and trying to document what changes are done in solana codebase by vaempire.

Short algorithm describing how to handle full update:
1. Pointing out what files was changed in vaempire since last full-update.
...
4. Ignore files that wasnt modified by vaempire (checkout number 1 of this algorithm)
