...
RELEASE_BASENAME="${RELEASE_BASENAME:=vaempire-release}"
...
(
  ...
  cp "${RELEASE_BASENAME}"/bin/vaempire-install-init vaempire-install-init-$TARGET
...
for file in "${TARBALL_BASENAME}"-$TARGET.tar.bz2 "${TARBALL_BASENAME}"-$TARGET.yml vaempire-install-init-"$TARGET"* $MAYBE_TARBALLS; do
