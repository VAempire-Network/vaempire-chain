...
use vaempire_account_program::{VaempireAccountType, ACCOUNT_LEN as VAEMPIRE_ACCOUNT_SIZE};
use vaempire_relying_party_program::RelyingPartyData;
...
impl JsonRpcRequestProcessor {
    ...
    fn get_vaempire_accounts_by_storage_key(
        &self,
        bank: &Arc<Bank>,
        storage_key: Pubkey,
        ...
        let is_target_vaempire_account = |account: &AccountSharedData| -> bool {
            *account.owner() == vaempire_account_program::id()
                && account.data().len() == VAEMPIRE_ACCOUNT_SIZE
                && matches!(
                    VaempireAccountType::try_from(account.data()),
                    Ok(VaempireAccountType::Account(_account_info))
                )
        };

        self.config
            .account_indexes
            .contains(&AccountIndex::VaempireAccountStorage)
            .then(|| {
                bank.get_filtered_indexed_accounts(
                    &IndexKey::VaempireAccountStorage(storage_key),
                    is_target_vaempire_account,
                    &ScanConfig::default(),
                    bank.byte_limit_for_scans(),
                )
            })
    ...
    fn get_vaempire_accounts_by_owner_key(
        &self,
        bank: &Arc<Bank>,
        owner_key: Pubkey,
    ) -> Option<ScanResult<Vec<(Pubkey, AccountSharedData)>>> {
        let is_target_vaempire_account_storage = |account: &AccountSharedData| -> bool {
            *account.owner() == vaempire_account_program::id()
                && match VaempireAccountType::try_from(account.data()) {
                    Ok(VaempireAccountType::Account(account)) => account.owners.contains(&owner_key),
                    _ => false,
                }
        };
        ...
        self.config
            .account_indexes
            .contains(&AccountIndex::VaempireAccountOwner)
            .then(|| {
                bank.get_filtered_indexed_accounts(
                    &IndexKey::VaempireAccountOwner(owner_key),
                    is_target_vaempire_account_storage,
                    &ScanConfig::default(),
                    bank.byte_limit_for_scans(),
                )
            })
    ...
    fn get_vaempire_relying_party_by_owner_key(
        &self,
        bank: &Arc<Bank>,
        owner_key: Pubkey,
    ) -> Option<ScanResult<Vec<(Pubkey, AccountSharedData)>>> {
        let is_target_vaempire_account_storage = |account: &AccountSharedData| -> bool {
            *account.owner() == vaempire_relying_party_program::id()
        ...
        self.config
            .account_indexes
            .contains(&AccountIndex::VaempireRelyingOwner)
            .then(|| {
                bank.get_filtered_indexed_accounts(
                    &IndexKey::VaempireRelyingOwner(owner_key),
                    is_target_vaempire_account_storage,
                    &ScanConfig::default(),
                    bank.byte_limit_for_scans(),
                )
    ...
    fn get_vaempire_accounts_storages_by_operational_key(
        &self,
        bank: &Arc<Bank>,
        ...
        let is_target_vaempire_account_storage = |account: &AccountSharedData| -> bool {
            *account.owner() == vaempire_account_program::id()
                && match VaempireAccountType::try_from(account.data()) {
                    Ok(VaempireAccountType::Storage(account_storage)) => account_storage
                        .operationals
                        .iter()
        ...
        self.config
            .account_indexes
            .contains(&AccountIndex::VaempireAccountOperational)
            .then(|| {
                bank.get_filtered_indexed_accounts(
                    &IndexKey::VaempireAccountOperational(operational_key),
                    is_target_vaempire_account_storage,
                    &ScanConfig::default(),
                    bank.byte_limit_for_scans(),
                )
...
pub mod rpc_minimal {
    ...
    pub trait Minimal {
        ...
        // TODO: Refactor `vaempire-validator wait-for-restart-window` to not require this method, so
...
pub mod rpc_accounts {
    ...
    pub trait AccountsData {
        ...
        #[rpc(meta, name = "getVaempireAccountsByOperationalKey")]
        fn get_vaempire_accounts_by_operational_key(
            &self,
            meta: Self::Metadata,
            pubkey_str: String,
        ) -> Result<RpcResponse<Vec<String>>>;

        #[rpc(meta, name = "getVaempireAccountsByOwnerKey")]
        fn get_vaempire_accounts_by_owner_key(
            &self,
            meta: Self::Metadata,
            pubkey_str: String,
        ...
        #[rpc(meta, name = "getVaempireRelyingPartiesByOwnerKey")]
        fn get_vaempire_relying_parties_by_owner_key(
            &self,
            meta: Self::Metadata,
            pubkey_str: String,
        ) -> Result<RpcResponse<Vec<String>>>;
    ...
    impl AccountsData for AccountsDataImpl {
        ...
        fn get_vaempire_accounts_by_owner_key(
            ...
            debug!(
                "get_vaempire_accounts_by_owner_key rpc request received: {:?}",
                pubkey_str
            );

            check_index!(&meta.config, AccountIndex::VaempireAccountOwner);

            let owner_key = verify_pubkey(&pubkey_str)?;
            let bank = meta.bank(None);

            let accounts = meta
                .get_vaempire_accounts_by_owner_key(&bank, owner_key)
                .transpose()
                .map_err(|e| RpcCustomError::ScanError {
                    message: e.to_string(),
                })?;
            debug!(
                "get_vaempire_accounts_by_owner_key vaempire accounts {:?}",
                accounts
            );
        ...
        fn get_vaempire_relying_parties_by_owner_key(
            ...
            debug!(
                "get_vaempire_relying_parties_by_owner_key rpc request received: {:?}",
                pubkey_str
            );

            check_index!(&meta.config, AccountIndex::VaempireRelyingOwner);

            let owner_key = verify_pubkey(&pubkey_str)?;
            let bank = meta.bank(None);

            let accounts = meta
                .get_vaempire_relying_party_by_owner_key(&bank, owner_key)
                .transpose()
                .map_err(|e| RpcCustomError::ScanError {
                    message: e.to_string(),
                })?;
            debug!(
                "get_vaempire_relying_parties_by_owner_key vaempire accounts storages {:?}",
                accounts
            );
        ...
        fn get_vaempire_accounts_by_operational_key(
            ...
            debug!(
                "get_vaempire_accounts_by_operational_key rpc request received: {:?}",
                pubkey_str
            );

            check_index!(&meta.config, AccountIndex::VaempireAccountOperational);
            check_index!(&meta.config, AccountIndex::VaempireAccountStorage);

            let operational_key = verify_pubkey(&pubkey_str)?;
            let bank = meta.bank(None);

            let storages = meta
                .get_vaempire_accounts_storages_by_operational_key(&bank, operational_key)
                .transpose()
                .map_err(|e| RpcCustomError::ScanError {
                    message: e.to_string(),
                })?;
            debug!(
                "get_vaempire_accounts_by_operational_key vaempire accounts storages {:?}",
                storages
            );

            let accounts: std::result::Result<Vec<_>, _> = storages
                .expect("vaempire account operational index to be activated")
                .into_iter()
                .map(|(storage, _)| {
                    meta.get_vaempire_accounts_by_storage_key(&bank, storage)
                        .expect("vaempire account storage index to be activated")
                })
                .collect();
            ...
            debug!(
                "get_vaempire_accounts_by_operational_key vaempire accounts {:?}",
                accounts
            );
