...
impl<D: VaempireDBCommon> Clone for Storage<D> {
    fn clone(&self) -> Self {
        Self {
...
impl<D: VaempireDBCommon> std::fmt::Debug for Storage<D> {
...
static SECONDARY_MODE_PATH_SUFFIX: &str = "vaempire-secondary";
...
pub trait VaempireDBCommon: DBAccess + std::fmt::Debug + Sized {
...
impl VaempireDBCommon for rocksdb::DBWithThreadMode<rocksdb::SingleThreaded> {
    fn flush(&self) -> Result<(), rocksdb::Error> {
        self.flush()
    }
...
impl VaempireDBCommon for OptimisticTransactionDB {
    fn flush(&self) -> Result<(), rocksdb::Error> {
        self.flush()
    }
...
pub struct DbWithClose<D: VaempireDBCommon>(D);
...
impl<D: VaempireDBCommon> std::fmt::Debug for DbWithClose<D> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
